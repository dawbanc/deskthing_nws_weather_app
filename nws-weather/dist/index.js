...
__export(server_exports, {
  DeskThing: () => import_deskthing_server4.DeskThing
});
module.exports = __toCommonJS(server_exports);
var import_deskthing_server4 = __toESM(require_dist(), 1);

// server/settings.ts
var import_deskthing_server = __toESM(require_dist(), 1);
var setupSettings = async (Data) => {
  ...
  if (!hasAllSettings) {
    const Settings = {
      ...
      color: {
        ...
        type: "color",
        value: "black"
        // Will end up being a HEX code. This is just the default data
      }
    };
    import_deskthing_server.DeskThing.addSettings(Settings);
  }
};
...
var userInput = (Data) => {
  if (!Data?.user_input || !Data?.second_user_input) {
    ...
    import_deskthing_server2.DeskThing.getUserInput(requestScopes, async (data) => {
      if (data.payload.user_input && data.payload.second_user_input) {
        import_deskthing_server2.DeskThing.saveData(data.payload);
      } else {
        import_deskthing_server2.DeskThing.sendError(
          "Please fill out all the fields! Restart to try again"
        );
      }
    });
  } else {
    import_deskthing_server2.DeskThing.sendLog("Data Exists!");
  }
};

// server/sendingData.ts
var import_deskthing_server3 = __toESM(require_dist(), 1);
var sendSampleData = () => {
  import_deskthing_server3.DeskThing.send({ type: "sampleData", payload: "sampleData" });
};
var sendImage = async () => {
  const settings = await import_deskthing_server3.DeskThing.getSettings();
  if (settings && settings.image.type == "string" && settings.image.value) {
    const imageUrl = await import_deskthing_server3.DeskThing.saveImageReferenceFromURL(settings.image.value);
    import_deskthing_server3.DeskThing.send({ type: "image", payload: imageUrl || "" });
  }
};

// server/index.ts
var start = async () => {
  const Data = await import_deskthing_server4.DeskThing.getData();
  setupSettings(Data);
  userInput(Data);
};
var stop = async () => {
  import_deskthing_server4.DeskThing.sendLog("Server Stopped");
};
import_deskthing_server4.DeskThing.on("start", start);
import_deskthing_server4.DeskThing.on("stop", stop);
var handleRequest = async (socketData) => {
  switch (socketData.request) {
    ...
    case "image":
      ...
      break;
    default:
      import_deskthing_server4.DeskThing.sendError("Invalid Request");
      break;
  }
};
import_deskthing_server4.DeskThing.on("get", handleRequest);
